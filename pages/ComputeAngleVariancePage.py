...
from functions.compute_angle_var import ComputeAngleVar
...
class ComputeAngleVarFrame(tb.Frame):
  def __init__(self, parentFrame):
    ...
    self.label = tb.Label(self, text="COMPUTE RPY VARIANCE", font=('Monospace',16, 'bold') ,bootstyle="dark")
    
    self.frame = tb.Frame(self)
    
    self.computeAngleVar = ComputeAngleVar()
    ...
    self.computeAngleVarButton = tb.Button(self.frame, text="COMPUTE RPY VARIANCE",
                               style=buttonStyleName, padding=20,
                               command=self.runComputation)
    

    #add framed widgets to frame
    self.computeAngleVarButton.pack(side='top', expand=True, fill="both")
